...
function optimizeMakerSplitterPairs(components: ComponentInstance[], nets: Map<NetBitId, NetBit>): ComponentInstance[] {
  for (const comp of components) {
    ...
    if (isRedundant && sourceSplitterId) {
      ...
      if (splitter) {
        ...
        if (typeof splitterInNetId === "string" && typeof makerOutNetId === "string") {
          ...
          if (splitterInNet && makerOutNet && splitterInNet !== makerOutNet) {
            ...
            for (const sink of makerOutNet.sinks) {
              ...
              const sinkComp = components.find((c) => c.id === sink.componentId);
              if (sinkComp) {
...
function cleanupRedundantComponents(components: ComponentInstance[], nets: Map<NetBitId, NetBit>): ComponentInstance[] {
  ...
  while (changed) {
    ...
    for (const comp of currentComponents) {
      if (comp.template.id.startsWith("SPLITTER_") || comp.template.id.startsWith("MAKER_")) {
...
export function buildNetlistFromYosys(json: unknown, options: YosysAdapterOptions): NetlistGraph {
  ...
  for (const [cellName, cell] of Object.entries(module.cells ?? {})) {
    ...
    if (cell.type === "$dffe") {
      ...
      // 2. Enable (EN)
      let enWire = normalizeBit(ensureArray(cell.connections["EN"], "EN", 1)[0], counter).id;
      if (enPol === 0) {
        // Invert Enable
    ...
    if (cell.type === "$reduce_bool") {
       ...
       if (size === 1) {
          const constInst = instantiate(getTemplate("CONST_0"), `${instanceId}_zero`);
          components.push(constInst);
          constBusId = `${instanceId}_zero_out`;
          constInst.connections["out"] = constBusId;
          registerSource(nets, constBusId, {componentId: `${instanceId}_zero`, portId: "out"});
       } else {
          const constInst = instantiate(getTemplate(`CONST_${size}`), `${instanceId}_zero`);
          if (!constInst.metadata) constInst.metadata = {};
    ...
    if (cell.type === "$reduce_and") {
       ...
       if (size === 1) {
          ...
          const constInst = instantiate(getTemplate("CONST_1"), `${instanceId}_ones`);
          components.push(constInst);
          constBusId = `${instanceId}_ones_out`;
          constInst.connections["out"] = constBusId;
          registerSource(nets, constBusId, {componentId: `${instanceId}_ones`, portId: "out"});
       } else {
          const constInst = instantiate(getTemplate(`CONST_${size}`), `${instanceId}_ones`);
          if (!constInst.metadata) constInst.metadata = {};
    ...
    if (cell.type === "$logic_and") {
      ...
      const getNonZeroBit = (inRaw: any[], width: number, size: number, suffix: string): string => {
         if (width === 1) {
            ...
            const aPacked = packBits(inRaw.map(b => normalizeBit(b, counter)), size, components, nets, idGen);
            eqInst.connections["A"] = aPacked;
            registerSink(nets, aPacked, {componentId: eqId, portId: "A"});
